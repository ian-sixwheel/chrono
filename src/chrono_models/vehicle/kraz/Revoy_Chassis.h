...
public:
  ...
  /// Get the location (in the local frame of this chassis) of the connection to
  /// the rear chassis.
  virtual const ChVector3d GetLocalPosRearConnector() const override {
    return m_rear_connector_loc;
  }

  /// Get the local driver position and orientation.
  /// This is a coordinate system relative to the chassis reference frame.
  // virtual ChCoordsys<> GetLocalDriverCoordsys() const override { return
  // m_driverCsys; }

protected:
  virtual double GetBodyMass() const override { return m_body_mass; }
  virtual ChMatrix33<> GetBodyInertia() const override {
    return m_body_inertia;
  }
  virtual ChFrame<> GetBodyCOMFrame() const override {
    return ChFrame<>(m_body_COM_loc, QUNIT);
  }
...
public:
  Revoy_Connector(const std::string &name)
      : ChChassisConnectorFifthWheel(name) {}
  ChVector3d GetHitchForce() { return m_joint->GetReaction2().force; }
  ~Revoy_Connector() {}
};
